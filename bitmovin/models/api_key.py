...
class ApiKey(BitmovinResponse):
    ...
    def openapi_types(self):
        types = super(ApiKey, self).openapi_types
        types.update({
            'key_value': 'str'
        })
        return types
    ...
    def attribute_map(self):
        attributes = super(ApiKey, self).attribute_map
        attributes.update({
            'key_value': 'keyValue'
        })
        return attributes

    def __init__(self, key_value=None, *args, **kwargs):
    ...
    def key_value(self, key_value):
        ...
        if key_value is not None:
            if not isinstance(key_value, str):
                raise TypeError("Invalid type for `key_value`, type has to be `str`")

            self._key_value = key_value

    def to_dict(self):
        """Returns the model properties as a dict"""
        result = super(ApiKey, self).to_dict()
